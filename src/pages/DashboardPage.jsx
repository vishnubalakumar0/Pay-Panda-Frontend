...
export default function DashboardPage() {
  ...
  const {toast}=useUi();
  useEffect(() => { setLoading(true); const suffix=unitId?`&business_unit_id=${unitId}`:''; Promise.all([api.get(`/dashboard/summary${unitId?`?business_unit_id=${unitId}`:''}`), api.get(`/dashboard/payments?limit=6${suffix}`),api.get('/clients')]).then(([a,b,c]) => { setSummary(a.data.summary); setPayments(b.data.payments);setClientCount(c.data.clients.length); }).catch(()=>toast('Could not load dashboard data','error')).finally(()=>setLoading(false)); }, [toast,unitId]);
  ...
  return <div ref={rootRef}>
    ...
    {summary?.businessUnits?.length ? <section className="panel unit-filter-panel"><label>View summary for<select value={unitId} onChange={e=>setUnitId(e.target.value)}><option value="">All sub-businesses</option>{summary.businessUnits.map(unit=><option key={unit.id} value={unit.id}>{unit.name}</option>)}</select></label><Link to="/business-units">Manage sub-businesses</Link></section> : null}
    ...
    <section className={`dashboard-grid ${summary?.lifetime?.count ? 'checklist-complete' : ''}`}><article className="panel activity-panel"><div className="panel-heading"><div><h3>Recent payments</h3><p>Latest sessions created through API and dashboard.</p></div><Link to="/payments/history">View all</Link></div>
      <div className="table-wrap"><table><thead><tr><th>Order</th><th>Sub-business</th><th>Customer</th><th>Amount</th><th>Status</th></tr></thead><tbody>{loading ? <tr><td colSpan="5" className="empty-cell"><RefreshCw className="spin"/> Loading payments…</td></tr> : payments.length ? payments.map(p => <tr key={p.id}><td><strong>{p.clientOrderId}</strong><small>{new Date(p.createdAt).toLocaleString()}</small></td><td>{p.businessUnit?.name || 'Main'}</td><td>{p.customerName || 'Guest'}</td><td>₹{Number(p.amount).toFixed(2)}</td><td><span className={`status status-${p.status.toLowerCase()}`}><i/>{p.status}</span></td></tr>) : <tr><td colSpan="5" className="empty-cell">No payments yet.</td></tr>}</tbody></table></div>
